...
```ore
// Function declaration with fn keyword and colon notation
fn add(a: int, b: int): int {
    return a + b
}

// Function declaration with fn keyword and arrow notation
fn multiply(a: int, b: int) -> int {
    return a * b
}

// Function declaration with function keyword and arrow notation
...
fn transform<T, U>(value: T, converter: (T) -> U) -> U {
...
@timeout(1000)  // Function will fail if it takes longer than 1000ms
...
// Point to string utility function using fn
fn pointToString(point): string {
    let (x, y) = point
    return f"({x}, {y})"
}

// Alternative using function keyword
...
// Function to find department by tag using fn
fn findDepartmentByTag(tag): list {
...
// Alternative using function keyword and arrow notation
function findEmployeesByDepartment(department: string) -> list {
    for (depTags, employees in employeesByDepartment) {
        if (depTags.contains(department)) {
            return employees
        }
    }
    return []
...
for (emp in findEmployeesByDepartment("Marketing")) {
    println(f"- {emp}")
}
```
...
```ore
// Define an async function with fn keyword and colon notation
...
// Using arrow notation
async fn getData(id: int) -> string {
    await sleep(50)
    return "Data for ID ${id}"
}

// Using function keyword
async function retrieveUserData(userId: string) -> object {
    await sleep(75)
...
// Using function keyword and arrow notation
async function fetchWithTimeout(url: string, timeout: int) -> string {
    try {
...
```ore
// Using fn keyword and colon notation
@memoize
fn fibonacci(n: int): int {
    if (n <= 1) return n
    return fibonacci(n-1) + fibonacci(n-2)
...
```ore
// Using fn keyword and colon notation
fn processLogFile(path: string): void {
    ...
    with (let reportFile = File.open("error_report.txt", "w")) {
        reportFile.write(f"Error Report for {path}\n")
        reportFile.write(f"Total errors found: {errorCount}\n\n")
        
        for (errorLine in errorLines) {
            reportFile.write(f"{errorLine}\n")
        }
    }
    
    print("Report generated: error_report.txt")
    print(f"Total errors found: {errorCount}")
}
...
function main(argc, args) -> int {
    ...
    try {
        ...
        print(f"File {filename} has {lines.length} lines")
...
```ore
// Using async function with arrow notation
async function main(argc, args) -> void {
    try {
        ...
        print(f"Received {parsed.items.length} items")
...
// Using async fn with colon notation
async fn fetchData(url: string): string {
    let resp = await http.get(url)
    if (resp.status != 200) {
        throw f"HTTP Error: {resp.status}"
    }
    return resp.body
}
```
